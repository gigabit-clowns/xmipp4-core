...
/**
 * @brief A batch of regions to be written, ready to be walked.
 *
 * A write writes the file, so the file is named first in the layout.
 * That is what settles the order the axes are walked in, and so makes the
 * traversal sequential in the file.
 *
 * That is the whole of what separates this from
 * @ref mrc_region_read_plan, and it is why the two are separate types
 ...
 * @ref write_regions accepts this one.
 */
class mrc_region_write_plan
{
public:
	/**
	...
	mrc_region_write_plan(const mrc_region_write_plan &other) = delete;
	mrc_region_write_plan(mrc_region_write_plan &&other) noexcept = default;
	~mrc_region_write_plan() = default;

	mrc_region_write_plan&
	operator=(const mrc_region_write_plan &other) = delete;
	mrc_region_write_plan&
	operator=(mrc_region_write_plan &&other) noexcept = default;
	 ...
	 *
	 * @return const joint_layout& The layout, naming the file first.
	 */
