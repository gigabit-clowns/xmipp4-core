...
TEST_CASE( "default constructed host_receive_region should be empty ", "[host_receive_region]" ) 
{
...
TEST_CASE( "constructing a host_receive_region from a raw pointer should store its attributes", "[host_receive_region]" ) 
{
...
TEST_CASE( "constructing a host_receive_region from a statically typed pointer store its attributes", "[host_receive_region]" ) 
{
	std::array<float32_t, 1024> data = {}; 
	const host_receive_region region(
		data.data(),
		data.size()
	);
