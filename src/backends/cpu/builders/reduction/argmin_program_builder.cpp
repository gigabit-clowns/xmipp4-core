 ...
 * found at is dropped.
 *
 * There is no identity. A smallest element of nothing does not exist, and the
...
struct argmin_kernel
{
	...
	{
		// Strictly smaller, so that a tie keeps the first place it was seen.
...
XMIPP4_REGISTER_INDEXED_REDUCTION_PROGRAM_BUILDER(
	...
	default_kernel_factory<argmin_kernel>
);
