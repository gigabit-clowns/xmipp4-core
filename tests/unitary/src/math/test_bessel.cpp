...
TEMPLATE_TEST_CASE( "cylindrical_bessel_y1 should produce correct numerical results", "[math]", float, double ) 
{
    ...
    std::tie(x, expected) = GENERATE(
        table<TestType, TestType>({
            // {0.0, -std::numeric_limits<TestType>::infinity()}, // On Windows returns NaN
...
TEMPLATE_TEST_CASE( "cylindrical_bessel_yn should produce correct numerical results", "[math]", float, double ) 
{
    ...
    std::tie(n, x, expected) = GENERATE(
        table<int, TestType, TestType>({
            // {{2, 0.0}, -std::numeric_limits<TestType>::infinity()}, // On Windows returns NaN
            // {{3, 0.0}, -std::numeric_limits<TestType>::infinity()}, // On Windows returns NaN
...
TEMPLATE_TEST_CASE( "cylindrical_bessel_kn should produce correct numerical results", "[math]", float, double ) 
{
    ...
    std::tie(n, x, expected) = GENERATE(
        table<int, TestType, TestType>({
            {2, 0.0, std::numeric_limits<TestType>::infinity()},
            {3, 0.0, std::numeric_limits<TestType>::infinity()},
