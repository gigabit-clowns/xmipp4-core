...
struct add_kernel
{
	...
	) const
	{
		...
		auto x_ite = make_strided_pointer_iterator(x, x_stride);
		auto y_ite = make_strided_pointer_iterator(y, y_stride);
		for (std::size_t i = 0; i < count; ++i)
		{
			*result_ite = *x_ite + *y_ite;
			++result_ite;
			++x_ite;
			++y_ite;
		}
	...
	void operator()(
		...
		const T* y,
		std::size_t count,
		contiguous_stride_tag /*result_stride*/, 
		contiguous_stride_tag /*x_stride*/, 
		contiguous_stride_tag /*y_stride*/
	) const
	{
		for (std::size_t i = 0; i < count; ++i)
		{
