...
class eager_operation_dispatcher_fixture
{
public:
	eager_operation_dispatcher_fixture()
		...
		, manager(std::make_shared<operation_program_manager>())
		, program(std::make_shared<hardware::mock_program>())
	{
		...
		dispatcher = make_eager_operation_dispatcher(manager);
...
protected:
	...
	std::shared_ptr<operation_program_manager> manager;
	std::shared_ptr<operation_dispatcher> dispatcher;
...
TEST_CASE(
	"eager_operation_dispatcher dispatch throws when the device context has no "
	"active queue",
	"[eager_operation_dispatcher]"
)
{
	const auto dispatcher = make_eager_operation_dispatcher(
		std::make_shared<operation_program_manager>()
	);
...
TEST_CASE_METHOD(
	eager_operation_dispatcher_fixture,
	"eager_operation_dispatcher dispatch throws when the operand counts do not "
...
TEST_CASE_METHOD(
	eager_operation_dispatcher_fixture,
	"eager_operation_dispatcher dispatch throws when an input operand lacks "
	"storage",
	"[eager_operation_dispatcher]"
)
{
...
TEST_CASE_METHOD(
	eager_operation_dispatcher_fixture,
	"eager_operation_dispatcher dispatch builds the program, allocates output "
...
TEST_CASE_METHOD(
	eager_operation_dispatcher_fixture,
	"eager_operation_dispatcher dispatch reuses a matching pre-allocated output "
...
TEST_CASE_METHOD(
	eager_operation_dispatcher_fixture,
	"eager_operation_dispatcher dispatch allocates the scratch buffers required "
