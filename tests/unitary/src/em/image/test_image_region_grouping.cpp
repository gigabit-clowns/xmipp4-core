...
TEST_CASE( "an image_region_grouping starts empty", "[image_region_grouping]" )
{
	...
	REQUIRE( grouping.get_region_count() == 0 );
	REQUIRE( grouping.get_file_count() == 0 );
}

TEST_CASE( "an image_region_grouping gathers the regions of each file together",
	"[image_region_grouping]" )
{
	...
	{
		REQUIRE( plan.get_region_count() == 5 );
	...
	{
		REQUIRE( grouping.get_region_count() == 5 );
		REQUIRE( grouping.get_file_count() == 3 );
		REQUIRE( grouping.get_file_region_count(zero) == 2 );
		REQUIRE( grouping.get_file_region_count(one) == 2 );
		REQUIRE( grouping.get_file_region_count(two) == 1 );
	...
	SECTION( "every region appears exactly once" )
	{
		...
		for (std::size_t i = 0; i < grouping.get_region_count(); ++i)
		{
	...
	SECTION( "building again picks up regions added since" )
	{
		...
		REQUIRE( grouping.get_region_count() == 6 );
		REQUIRE( grouping.get_file_region_count(two) == 2 );
	}
}
	...
	"[image_region_grouping]" )
{
	...
	REQUIRE( grouping.get_file_region_count(empty) == 0 );
...
TEST_CASE( "an empty plan groups into nothing", "[image_region_grouping]" )
{
	...
	REQUIRE( grouping.get_file_region_count(0) == 0 );
	REQUIRE( grouping.get_first_position(0) == 0 );
}

TEST_CASE(
	"an image_region_grouping reused across transactions stops allocating",
	"[image_region_grouping]"
)
{
	...
	for (std::size_t repeat = 0; repeat < 8; ++repeat)
	{
		...
		REQUIRE( grouping.get_region_count() == count );
		REQUIRE( grouping.get_file_region_count(0) == count / 2 );
...
TEST_CASE(
	"an image_region_grouping has value semantics",
	"[image_region_grouping]"
)
{
	image_transaction_plan plan(make_span(plane_extents), 3, 3);
	...
	{
		const image_region_grouping copy(grouping);
	...
	{
		image_region_grouping copy(grouping);
		copy.clear();

		REQUIRE( copy.get_region_count() == 0 );
		REQUIRE( grouping.get_region_count() == 1 );
	}
}
