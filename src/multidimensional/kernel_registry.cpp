...
{
public:
    bool register_kernel_builder(operation_id key,
    ...
    get_kernel_builder(operation_id key, const compute::device_backend &backend) const noexcept
    {
...
private:
    using key_type = kernel_key;
    using value_type = std::unique_ptr<kernel_builder>;

    std::unordered_map<key_type, value_type> m_builders;
...
bool kernel_registry::register_kernel_builder(operation_id key,
                                              const compute::device_backend &backend,
                                              std::unique_ptr<kernel_builder> builder )
{
...
const kernel_builder * 
kernel_registry::get_kernel_builder(operation_id key,
