 ...
 *
 * The @ref cpu_program interface is type erased: its @ref cpu_program::execute
...
public:
	...
	>
	explicit functor_cpu_program(functor_type functor = {});
...
/**
 * @brief Create a @ref functor_cpu_program that requires no scratch buffers.
 *
 ...
 * @return The wrapped program, held through a `std::shared_ptr<cpu_program>`.
 */
template <typename F, typename Outputs, typename Inputs>
std::shared_ptr<cpu_program> make_functor_cpu_program(
	F &&fun,
...
/**
 * @brief Create a @ref functor_cpu_program with scratch buffers.
 *
