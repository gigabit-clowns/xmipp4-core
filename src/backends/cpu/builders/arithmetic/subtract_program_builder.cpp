	...
	return make_functor_program(
		...
		(std::tuple<T*> outputs, std::tuple<const T*, const T*> inputs, std::tuple<>)
		{
			run_elementwise_loop(
				...
				std::get<ops::subtract_operation::INPUT_OPERAND_LEFT>(inputs),
				std::get<ops::subtract_operation::INPUT_OPERAND_RIGHT>(inputs)
			);
		},
		type_list<T>(),
		type_list<T, T>()
...
) const
{
	...
	const auto &x_descriptor =
		input_signatures[ops::subtract_operation::INPUT_OPERAND_LEFT]
		.get_descriptor();
	const auto &y_descriptor =
		input_signatures[ops::subtract_operation::INPUT_OPERAND_RIGHT]
		.get_descriptor();
