...
class XMIPP4_CORE_API fallback_device_communicator_backend final
    : public device_communicator_backend
{
public:
    virtual bool supports_devices(span<device*> devices) const noexcept override;

    void create_device_communicators(
        const std::shared_ptr<communication::communicator> &node_communicator,
        span<device*> devices,
        std::vector<std::unique_ptr<device_communicator>> &result
    ) const override;
    
    void create_device_communicators_shared(
        const std::shared_ptr<communication::communicator> &node_communicator,
        span<device*> devices,
