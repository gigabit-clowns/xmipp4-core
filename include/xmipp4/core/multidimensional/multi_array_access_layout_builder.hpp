// SPDX-License-Identifier: GPL-3.0-only

#pragma once

#include "multi_array_access_layout_build_flags.hpp"
#include "multi_array_access_layout.hpp"
#include "../span.hpp"
#include "../platform/dynamic_shared_object.h"

#include <memory>

namespace xmipp4 
{
namespace multidimensional
{

class strided_layout;
class multi_array_access_layout_implementation;

/**
 * @brief Factory class to obtain instances of array_access_layout.
 * 
 * @see array_access_layout
 * 
 */
class multi_array_access_layout_builder
{
public:
	static XMIPP4_CONST_CONSTEXPR 
	multi_array_access_layout_build_flags default_flags = {
		multi_array_access_layout_build_flag_bits::enable_reordering,
		multi_array_access_layout_build_flag_bits::enable_coalescing
	};

	XMIPP4_CORE_API multi_array_access_layout_builder() noexcept;
	multi_array_access_layout_builder(
		const multi_array_access_layout_builder&
	) = delete;
	XMIPP4_CORE_API
	multi_array_access_layout_builder(
		multi_array_access_layout_builder&& other
	) noexcept;
	XMIPP4_CORE_API ~multi_array_access_layout_builder();

	multi_array_access_layout_builder& 
	operator=(const multi_array_access_layout_builder&) = delete;
	XMIPP4_CORE_API multi_array_access_layout_builder& 
	operator=(multi_array_access_layout_builder&& other) noexcept;

	/**
	 * @brief Set the extents of the iteration space.
	 * 
	 * @param extents Extents of the iteration space.
	 * @return multi_array_access_layout_builder& A reference to *this.
	 * 
	 * @note This method can only be called once at most for each layout 
	 * creation and it must be called before any call to add_operand.
	 */
	XMIPP4_CORE_API
	multi_array_access_layout_builder& 
	set_extents(span<const std::size_t> extents);

	/**
	 * @brief Add an operand to the layout.
	 *
	 * If set_extents has not been called, the first call to this method will
	 * set the extents of the operand as the extents of the iteration space.
	 * 
	 * When the iteration space extents are defined, the provided layout must
	 * have the same extents.
	 *  
	 * @param layout Layout of the operand.
	 * @return multi_array_access_layout_builder& A reference to *this.
	 */
	XMIPP4_CORE_API
	multi_array_access_layout_builder& 
	add_operand(const strided_layout &layout);

	/**
	 * @brief Build the array_access_layout from the operands contained in 
	 * this builder.
	 * 
	 * The building process can apply certain optimizations to the joint
	 * layout which can be controlled though the flags parameter.
	 * 
	 * @param flags Flags controlling the optimizations applied to the joint
	 * layout. By default, all optimizations are enabled.
	 * @return array_access_layout The newly built array_access_layout.
	 */
	XMIPP4_CORE_API
	multi_array_access_layout build(
		multi_array_access_layout_build_flags flags = default_flags
	);

	/**
	 * @brief Get a pointer to the implementation.
	 * 
	 * @return const multi_array_access_layout_implementation* Pointer to the
	 * implementation.
	 * 
	 * @note This method is meant to be used for testing purposes, as the 
	 * multi_array_access_layout_implementation is not publicly available.
	 */
	const multi_array_access_layout_implementation* 
	get_implementation() const noexcept;

private:
	std::unique_ptr<multi_array_access_layout_implementation> m_implementation;
};

} // namespace multidimensional
} // namespace xmipp4
