...
TEST_CASE( "require same type throws", "[checks]" )
{
    ...
    REQUIRE_THROWS_WITH( require_same_type(numerical_type::int16, numerical_type::int32), "Both buffers must have the same numerical type" );
}
...
TEST_CASE( "require same count throws", "[checks]" )
{
    ...
    REQUIRE_THROWS_WITH( require_same_count(2, 3), "Both buffers must have the same element count" );
}
