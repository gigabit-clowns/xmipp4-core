...
)
{
	return resolve_gemm<T>(
		m, k, n, out_kind, left_kind, right_kind, eigen_fixed_path_support<T>{}
	);
}
...
// Every builder that multiplies matrices (matmul, matvec, vecmat) needs
// this for every type eigen_scalar_support admits, but only ever calls it
// once per program, at construction time. What is expensive is not that
// call but the fixed-size dispatch grid a "worth it" type (see
// eigen_fixed_path_support above) drives it through: two or three nested
// dispatch_extent calls, each branch of which names a distinct
// Eigen::Matrix and so a distinct instantiation of Eigen's own GEMM
// expression templates. Instantiating that grid here, once per element
// type, and explicitly, is what keeps every builder that resolves a gemm
// from re-instantiating it for itself, the same way fourier_transform.cpp
// keeps pocketfft out of its own callers.
template gemm_fn<std::int8_t> resolve_gemm<std::int8_t>(
	std::size_t, std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<std::uint8_t> resolve_gemm<std::uint8_t>(
	std::size_t, std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<std::int16_t> resolve_gemm<std::int16_t>(
	std::size_t, std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<std::uint16_t> resolve_gemm<std::uint16_t>(
	std::size_t, std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<std::int32_t> resolve_gemm<std::int32_t>(
	std::size_t, std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<std::uint32_t> resolve_gemm<std::uint32_t>(
	std::size_t, std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<std::int64_t> resolve_gemm<std::int64_t>(
	std::size_t, std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<std::uint64_t> resolve_gemm<std::uint64_t>(
	std::size_t, std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<float32_t> resolve_gemm<float32_t>(
	std::size_t, std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<float64_t> resolve_gemm<float64_t>(
	std::size_t, std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<std::complex<float32_t>> resolve_gemm<std::complex<float32_t>>(
	std::size_t, std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<std::complex<float64_t>> resolve_gemm<std::complex<float64_t>>(
	std::size_t, std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);

template gemm_fn<std::int8_t> resolve_gemv<std::int8_t>(
	std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<std::uint8_t> resolve_gemv<std::uint8_t>(
	std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<std::int16_t> resolve_gemv<std::int16_t>(
	std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<std::uint16_t> resolve_gemv<std::uint16_t>(
	std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<std::int32_t> resolve_gemv<std::int32_t>(
	std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<std::uint32_t> resolve_gemv<std::uint32_t>(
	std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<std::int64_t> resolve_gemv<std::int64_t>(
	std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<std::uint64_t> resolve_gemv<std::uint64_t>(
	std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<float32_t> resolve_gemv<float32_t>(
	std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<float64_t> resolve_gemv<float64_t>(
	std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<std::complex<float32_t>> resolve_gemv<std::complex<float32_t>>(
	std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<std::complex<float64_t>> resolve_gemv<std::complex<float64_t>>(
	std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);

template gemm_fn<std::int8_t> resolve_vecgemm<std::int8_t>(
	std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<std::uint8_t> resolve_vecgemm<std::uint8_t>(
	std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<std::int16_t> resolve_vecgemm<std::int16_t>(
	std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<std::uint16_t> resolve_vecgemm<std::uint16_t>(
	std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<std::int32_t> resolve_vecgemm<std::int32_t>(
	std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<std::uint32_t> resolve_vecgemm<std::uint32_t>(
	std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<std::int64_t> resolve_vecgemm<std::int64_t>(
	std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<std::uint64_t> resolve_vecgemm<std::uint64_t>(
	std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<float32_t> resolve_vecgemm<float32_t>(
	std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<float64_t> resolve_vecgemm<float64_t>(
	std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<std::complex<float32_t>> resolve_vecgemm<std::complex<float32_t>>(
	std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
template gemm_fn<std::complex<float64_t>> resolve_vecgemm<std::complex<float64_t>>(
	std::size_t, std::size_t,
	core_layout_kind, core_layout_kind, core_layout_kind
);
