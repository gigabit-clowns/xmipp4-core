...
TEST_CASE("execute should execute a properly configured kernel", "[operation_execute]")
{
	std::vector<std::unique_ptr<trompeloeil::expectation>> expectations;
...
TEST_CASE("execute should throw if an storage-less input array is provided", "[operation_execute]")
{
	service_catalog catalog(false);
	auto device = std::make_shared<mock_device>();
	mock_memory_resource device_resource;
