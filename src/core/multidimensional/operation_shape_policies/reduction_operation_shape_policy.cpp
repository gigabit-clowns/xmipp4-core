...
void reduction_operation_shape_policy::infer_output(
...
) const
{
	if (input_layouts.empty())
	{
		throw std::invalid_argument(
			"reduction_operation_shape_policy::infer_output requires at least one input"
		);
	}
...
void reduction_operation_shape_policy::validate(
...
) const
{
	...
	{
		throw std::invalid_argument(
			"reduction_operation_shape_policy::validate requires at least one output"
		);
	}
	...
	for (std::size_t i = 1; i < output_layouts.size(); ++i)
	{
		...
		{
			throw std::invalid_argument(
				"multi-output reduction_operation_shape_policy::validate requires all "
	...
	for (const auto &layout : output_layouts)
	{
		...
		for (std::size_t axis = 0; axis < strides.size(); ++axis)
		{
			...
			{
				throw std::invalid_argument(
					"reduction_operation_shape_policy::validate: reduction axis does not "
			...
			{
				throw std::invalid_argument(
					"reduction_operation_shape_policy::validate: non-reduction axis has "
...
bool reduction_operation_shape_policy::is_reduction_axis(std::size_t axis) const noexcept
{
	const auto ite = 
