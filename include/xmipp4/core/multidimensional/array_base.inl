...
std::size_t strided_array_base<T>::get_rank() const noexcept
{
...
{
    return strided_array_base(
        m_storage.view(), 
        m_layout.apply_subscripts(subscripts)
    );
}
...
strided_array_base<T>::swap_axes(std::size_t axis1, std::size_t axis2)
{
    return strided_array_base(
        m_storage.view(), 
        m_layout.swap_axes(axis1, axis2)
    );
}
